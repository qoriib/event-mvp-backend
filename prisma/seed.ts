...
async function main() {
  ...
  const sampleEvents = Array.from({ length: 12 }).map((_, i) => ({
    ...
    description:
      "Sebuah acara yang menampilkan berbagai aktivitas menarik dan hiburan live.",
  ...
  const randomTicket = () =>
    allTicketTypes[Math.floor(Math.random() * allTicketTypes.length)];

  const txStatuses = [
    TxStatus.DONE,
    TxStatus.WAITING_PAYMENT,
    TxStatus.WAITING_CONFIRMATION,
    TxStatus.REJECTED,
  ];
